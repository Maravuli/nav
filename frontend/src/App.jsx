...
const CAMPUS_GATES = [
  ...
  { id: "gate3", name: "Gate 3", lat: -23.89254776676242, lng: 29.73366717590656 },
  { id: "gate0", name: "Gate 0", lat: -23.880366045435327, lng: 29.74096031145133 }
];
...
function nearestGate(latlng) {
  ...
  for (const g of CAMPUS_GATES) {
    const d = haversine(latlng, [g.lat, g.lng]);
    if (d < bestD) {
      bestD = d;
      best = g;
    }
...
function mergeRouteParts(partA, partB, joinInstruction = null) {
  ...
  const coords = [...partA.coords];
  const bCoords = partB.coords;
  const last = coords[coords.length - 1];
  const firstB = bCoords[0];
  const skip = haversine(last, firstB) < 12 ? 1 : 0;
  for (let i = skip; i < bCoords.length; i++) coords.push(bCoords[i]);
  const totalDistance = Math.round(
    (partA.totalDistance || 0) + (partB.totalDistance || 0)
  );
  const totalTime = Math.max(
    1,
    (partA.totalTime || 0) + (partB.totalTime || 0)
  );
  let steps = [];
  if (partA.steps?.length) steps = steps.concat(partA.steps.filter((s) => s.type !== "arrive"));
  if (joinInstruction) {
    steps.push({
  ...
  if (partB.steps?.length) {
    const rest = partB.steps.filter((s) => s.type !== "depart");
  ...
  return {
    ...
    totalTime,
    steps,
    accessible: partA.accessible !== false && partB.accessible !== false,
    mode: partA.mode === "campus" || partB.mode === "campus" ? "campus" : partA.mode || partB.mode,
    travelMode: partA.travelMode || partB.travelMode
  };
}
...
function buildPathArrows(coords, spacingMeters = 55) {
  ...
  for (let i = 0; i < coords.length - 1; i++) {
    ...
    while (accumulated + leg >= nextAt) {
      const t = (nextAt - accumulated) / leg;
      const lat = a[0] + (b[0] - a[0]) * t;
      const lng = a[1] + (b[1] - a[1]) * t;
      arrows.push({ lat, lng, bearing: brng, kind: "path" });
...
function createDirectionArrowIcon(bearingDeg, { emphasize = false, turnType = null, mapBearing = 0 } = {}) {
  ...
  if (turnType === "left" || turnType === "slight-left" || turnType === "sharp-left") {
    ...
    pathSvg = `<path d="M12 6 L24 16 L12 26 L12 20 L4 20 L4 12 L12 12 Z" fill="white"/>`;
  ...
  const shadow = emphasize
    ? "0 3px 10px rgba(29,78,216,0.55)"
    : "0 2px 6px rgba(37,99,235,0.4)";
  return L.divIcon({
    className: `nav-dir-arrow ${emphasize ? "nav-dir-arrow-next" : ""}`,
    html: `<div class="nav-dir-arrow-inner" style="width:${size}px;height:${size}px;transform:rotate(${rot}deg);background:${bg};box-shadow:${shadow};">
      <svg width="${size}" height="${size}" viewBox="0 0 32 32" aria-hidden="true">${pathSvg}</svg>
    </div>`,
    iconSize: [size, size],
    iconAnchor: [size / 2, size / 2]
  });
  ...
  try {
    ...
    let steps = rawSteps
      ...
      .map((s) => {
        ...
        if (brng == null && stepCoords.length >= 2) {
          brng = bearing(stepCoords[0], stepCoords[Math.min(1, stepCoords.length - 1)]);
        }
        return {
          instruction: instruction.charAt(0).toUpperCase() + instruction.slice(1),
          distance: Math.round(s.distance || 0),
          type,
          lat: loc ? loc[1] : stepCoords[0]?.[0],
          lng: loc ? loc[0] : stepCoords[0]?.[1],
          bearing: brng ?? 0
        };
      });
    if (!steps.length) {
...
async function hybridRoute(fromLatLng, toLatLng, graphData, accessibleOnly, destinationName = "destination", travelMode = "walking") {
  ...
  const simpleLeg = (a, b, instruction) => {
    ...
    return {
      ...
      steps: [
        { instruction, distance: dist, type: "depart", lat: a[0], lng: a[1] },
        { instruction: `Continue`, distance: 0, type: "straight", lat: b[0], lng: b[1] }
      ],
      accessible: true,
      mode: "direct",
      travelMode
  ...
  if (travelMode === "walking" && !fromIn && toIn && graphData) {
    ...
    let toGate = withNamedSteps(await routeOSRM(fromLatLng, gateLL, "walking"));
    if (!toGate) toGate = simpleLeg(fromLatLng, gateLL, `Head to ${gate.name}`);
  ...
  if (travelMode === "walking" && fromIn && !toIn && graphData) {
    ...
    const gateLL = [gate.lat, gate.lng];
    const onCampus = routeOnCampusGraph(fromLatLng, gateLL, graphData, accessibleOnly, travelMode);
    if (onCampus) {
  ...
  if (fromIn && toIn) {
    ...
    if (travelMode === "walking" && graphData) {
      const { gate } = nearestGate(fromLatLng);
      const gateLL = [gate.lat, gate.lng];
      const a = routeOnCampusGraph(fromLatLng, gateLL, graphData, accessibleOnly, travelMode);
      const b = routeOnCampusGraph(gateLL, toLatLng, graphData, accessibleOnly, travelMode);
      if (a && b) {
        return withNamedSteps(
          mergeRouteParts(withNamedSteps(a), withNamedSteps(b), `Continue via ${gate.name}`)
        );
      }
    }
...
function LiveLocationController({ position, follow, zoomIn = false, heading = null }) {
  ...
  useEffect(() => {
    ...
    if (!didInitial.current) {
      ...
      const targetZoom =
        zoomIn && currentZoom < 17 ? Math.max(currentZoom, 17) : currentZoom;
  ...
  useEffect(() => {
    if (!follow) didInitial.current = false;
  }, [follow]);
  return null;
}
...
function StopFollowOnInteract({ active, onInteract }) {
  const map = useMap();
  useEffect(() => {
    if (!active || !onInteract) return;
    const stop = () => onInteract();
    ...
    <>
      ...
      {turnArrows.map((t, i) => (
        <Marker
          ...
          icon={createDirectionArrowIcon(t.bearing, {
            emphasize: t.isNext,
            turnType: t.type,
            mapBearing
          })}
...
function HeadingUpController({ heading, enabled, onUserRotate, onBearingChange, lockNorth = false }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (onBearingChange) onBearingChange(heading);
    } catch {
  ...
  }, [map, heading, enabled, onBearingChange]);
  ...
  useEffect(() => {
    ...
    const onRotate = () => {
      let b = 0;
      try {
        b = typeof map.getBearing === "function" ? map.getBearing() : 0;
      } catch {
        b = 0;
      }
      if (onBearingChange) onBearingChange(b);
      if (programmatic.current) return;
      if (onUserRotate) onUserRotate();
    };
  ...
  useEffect(() => {
    ...
    if (!enabled || lockNorth) {
      if (wasEnabled.current || lockNorth) {
        lastHeading.current = null;
        programmatic.current = true;
        try {
          map.setBearing(0);
          if (onBearingChange) onBearingChange(0);
        } catch {
          /* ignore */
        }
        setTimeout(() => {
  ...
  }, [map, enabled, lockNorth, onBearingChange]);
...
function NearbyPlaceLabels({
  ...
  if (!places?.length || zoom < minZoom) return null;

  const nearPoint = (lat, lng) => {
    const p = [lat, lng];
    if (userPos) {
      const d = haversine(userPos, p);
      if (d <= radiusMeters) return d;
    }
    if (routeCoords?.length) {
      let best = Infinity;
      const step = Math.max(1, Math.floor(routeCoords.length / 40));
      for (let i = 0; i < routeCoords.length; i += step) {
        const d = haversine(routeCoords[i], p);
        if (d < best) best = d;
      }
    ...
    if (!userPos && (!routeCoords || routeCoords.length < 2)) {
      try {
        ...
        if (b && b.contains([lat, lng])) return 0;
      } catch { /* ignore */ }
    }
    return null;
  };
  ...
  for (const place of places) {
    if (place?.lat == null || place?.lng == null || !place.name) continue;
    const d = nearPoint(place.lat, place.lng);
    if (d == null) continue;
    scored.push({ place, d });
  }
  scored.sort((a, b) => a.d - b.d);
  const visible = scored.slice(0, maxLabels);

  return (
    <>
      {visible.map(({ place }) => {
        const label = String(place.name).replace(/</g, "&lt;").replace(/>/g, "&gt;");
        const icon = L.divIcon({
          className: "building-name-label place-name-label",
          html: `<span class="building-name-label-text place-name-label-text">${label}</span>`,
          iconSize: [120, 20],
          iconAnchor: [60, 10]
        });
        return (
          <Marker
            key={`pl-${place.id || place.name}`}
...
function App() {
  ...
  const [showNavExitSheet, setShowNavExitSheet] = useState(false);
  ...
  const submitRating = (forcedValue = null, forcedComplaint = null) => {
    ...
    const complaintText = (forcedComplaint != null ? forcedComplaint : ratingComplaint || "").trim();
    if (!value) {
      return;
    }
  ...
  const reenableHeadingUp = useCallback(() => {
  ...
  const cancelDestination = () => {
    ...
    setNavToast("Destination canceled");
    window.setTimeout(() => setNavToast(""), 2800);
  };

  const confirmArrivedFromExit = () => {
    setShowNavExitSheet(false);
  ...
  useEffect(() => {
    ...
    if (route.coords?.length > 1) {
      ...
      near = remainingMeters <= 5;
    }
    const toLat = route.to.lat;
    const toLng = route.to.lng;
    if (toLat != null && toLng != null) {
      const straight = haversine(userPos, [toLat, toLng]);
      if (straight <= 5) near = true;
    }
  ...
  useEffect(() => {
    ...
    if (last?.lat != null && last?.lng != null) {
      ...
      if (dLast <= 25) active = steps.length - 1;
    }
                ...
                {isLogin && (
                  <button
                    ...
                    onClick={() => {
                      ...
                      setForgotSent(false);
                      setForgotMessage("");
                      setForgotStudentNumber((profileForm.studentNumber || "").trim());
                      setForgotEmail((profileForm.email || "").trim());
                    }}
                  >
                    Forgot password?
                  </button>
        ...
        {showForgotPassword && (
          ...
          >
            <div className="forgot-sheet" onClick={(e) => e.stopPropagation()}>
              <div className="forgot-sheet-header">
              ...
              {!forgotSent ? (
                <>
                  <p className="forgot-sheet-sub">
                    Enter your student number and the email on your account. We&apos;ll guide you on how to regain access.
                  </p>
                  <div className="form-group">
                    <label><IdCard size={14} /> Student number</label>
                    <input
                      type="text"
                      placeholder="e.g. 202012345"
                  ...
                  <div className="form-group">
                    <label><Mail size={14} /> Email on your account</label>
                    <input
                      type="email"
                      placeholder="name@ul.ac.za"
                      value={forgotEmail}
                  ...
                  {forgotMessage && (
                    <p className="rating-hint" style={{ marginBottom: 12 }}>{forgotMessage}</p>
                  )}
                  <button
                    type="button"
                    className="btn-primary full"
                    onClick={() => {
                      const sn = (forgotStudentNumber || "").trim();
                      const em = (forgotEmail || "").trim();
                      if (!sn) {
                        setForgotMessage("Enter your student number.");
                        return;
                      }
                      if (!em || !/[^\s@]+@[^\s@]+\.[^\s@]+/.test(em)) {
                ...
                <div className="forgot-success">
                  ...
                  <p>
                    If an account exists for <strong>{(forgotStudentNumber || "").trim().toUpperCase()}</strong>,
                    reset steps will be sent to <strong>{(forgotEmail || "").trim()}</strong>.
                  </p>
                  <p style={{ fontSize: 13, color: "#64748b" }}>
                    Check your inbox and spam folder. You can also contact UL ICT support if you need help.
                  </p>
                  <button
                    type="button"
                    className="btn-primary full"
  ...
  if (isNavigating && route) {
    ...
    const liveStepDist =
      useLiveAsFrom && userPos && step?.lat != null && step?.lng != null
        ...
        : null;
    const stepDistDisplay =
      liveStepDist != null ? liveStepDist : (step.distance > 0 ? step.distance : null);
    const formatDist = (m) => {
      ...
      <div className="mobile-screen nav-mode nav-fullscreen">
        <div className="nav-map-full">
          ...
          >
            ...
            {useLiveAsFrom && userPos && (
              <Marker
                key={`me-${followHeading ? "hu" : Math.round((navHeading ?? -1) / 5)}`}
                position={userPos}
                icon={createYouAreHereIcon(
                  // Heading-up: icon points up on screen (map already rotated).
                  // North-up: icon rotates by absolute heading.
            ...
            <HeadingUpController
              ...
              onUserRotate={stopHeadingOnUserRotate}
              onBearingChange={setMapBearing}
            />
            <StopFollowOnInteract
              active={followUser && useLiveAsFrom}
        ...
        <button
          ...
          aria-label={followHeading ? "Heading-up (following direction)" : "Recentre and face direction of travel"}
          title={followHeading ? "Heading-up on — map faces where you’re going" : "Recentre & face direction of travel"}
        >
          <Navigation size={20} />
        </button>
        <button
          type="button"
          className="nav-north-fab"
          onClick={() => {
            setFollowHeading(false);
            setMapBearing(0);
          }}
          aria-label="North up"
          title="North up (stop rotating with heading)"
        >
          <span className="nav-north-letter">N</span>
        </button>

        {nearDestination && !showNavExitSheet && (
        ...
        {showNavExitSheet && (
          <div className="nav-exit-overlay" role="dialog" aria-modal="true" aria-labelledby="nav-exit-title">
            <div className="nav-exit-sheet" onClick={(e) => e.stopPropagation()}>
              <h3 id="nav-exit-title">End navigation?</h3>
              <p className="nav-exit-sub">Choose how you want to finish this route.</p>
              <button type="button" className="btn-primary full nav-exit-arrived" onClick={confirmArrivedFromExit}>
                <Check size={18} /> I&apos;ve arrived
              </button>
              <button type="button" className="btn-outline full nav-exit-cancel" onClick={cancelDestination}>
                <X size={18} /> Cancel destination
              </button>
              <button
                type="button"
                className="nav-exit-keep"
                onClick={() => setShowNavExitSheet(false)}
              >
                Keep navigating
              </button>
            </div>
        ...
        <div className="nav-overlay-bottom">
          <div className="nav-instruction-card">
            <div className="nav-turn-badge" aria-hidden="true">
              <span className="nav-turn-glyph">{turnIconLabel(step.type)}</span>
            </div>
            <div className="nav-instruction-body">
              <span className="label">Next</span>
              <strong>{step.instruction}</strong>
              {stepDistDisplay != null && stepDistDisplay > 0 && (
  ...
  const renderRatingModal = () => {
    ...
    const canSubmit = ratingValue >= 1 && feedbackLen >= MIN_FEEDBACK_LEN;
    const starTone =
      ratingValue <= 0 ? "" :
      ratingValue <= 2 ? "tone-red" :
      ratingValue <= 4 ? "tone-yellow" :
      "tone-green";
    const starFill =
      ratingValue <= 0 ? UL_GOLD :
      ratingValue <= 2 ? "#ef4444" :
      ratingValue <= 4 ? "#eab308" :
      "#22c55e";
    return (
      <div className="rating-overlay" role="dialog" aria-modal="true" aria-labelledby="rating-title">
        <div className="rating-sheet" onClick={(e) => e.stopPropagation()}>
          <h3 id="rating-title">How was your route?</h3>
          <p className="rating-sub">
            {ratingRouteInfo?.to
              ? `You arrived at ${ratingRouteInfo.to}. Rate this navigation.`
              : "Rate this navigation experience."}
          </p>
          <div className={`rating-stars ${starTone}`}>
            {[1, 2, 3, 4, 5].map((n) => (
              <button
                key={n}
                type="button"
                className={`rating-star ${ratingValue >= n ? "on" : ""} ${starTone}`}
                onClick={() => handleStarSelect(n)}
                aria-label={`${n} star${n > 1 ? "s" : ""}`}
              >
                <Star size={36} fill={ratingValue >= n ? starFill : "none"} color={ratingValue >= n ? starFill : undefined} />
              </button>
            ))}
          </div>
          {ratingValue > 0 && (
            <div className="rating-complaint">
              <label>
                <MessageSquare size={14} /> Your feedback (required)
              </label>
              <textarea
                rows={3}
                placeholder="Tell us about your experience (required for all ratings)…"
          ...
          {ratingValue > 0 && feedbackLen < MIN_FEEDBACK_LEN && (
            <p className="rating-hint">Please write at least {MIN_FEEDBACK_LEN} characters of feedback before submitting.</p>
          )}
        </div>
      </div>
    );
            ...
            {CATEGORIES.map((c) => {
              ...
              return (
                ...
                >
                  ...
                  {isActive && (
                    <span
                      className="cat-item-check"
                      style={{ background: c.id === "all" ? "#ffffff" : c.color }}
                    />
                  )}
                </button>
              );
      ...
      <main className="main-content">
        ...
        <div className="map-wrapper">
          ...
          <MapContainer center={CAMPUS_CENTER} zoom={15} style={{ height: "100%", width: "100%" }} zoomControl={!isMobile} minZoom={11} maxZoom={19}>
            ...
            {userPos && (
              <Marker position={userPos} icon={createYouAreHereIcon(navDisplayHeading, theme === "dark")}>
            ...
            {userPos && (
              <CircleMarker
                ...
                radius={8}
                pathOptions={{
                  color: theme === "dark" ? UL_GOLD : UL_NAVY,
                  fillColor: UL_GOLD,
                  fillOpacity: theme === "dark" ? 0.28 : 0.2,
                  weight: theme === "dark" ? 2 : 1
                }}
              />
            )}
        ...
        {navToast && (
          <div className="app-toast app-toast-cancel" role="status">
            {navToast}
          </div>
        )}
      </main>
